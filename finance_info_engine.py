...
def timeSeries(API_key='****************', ticker="MSFT"):
...
def forex(API_key='****************', fromCurr='INR', toCurr='USD'):
    ...
    return a
inf1=forex(API_key='****************', fromCurr="USD", toCurr='INR')

def cryptocurrency(API_key='****************', currency="BTC"):
    ...
    return b
inf2=cryptocurrency(API_key='****************', currency="BTC")

def sectorPerformance(API_key='****************'):
...
inf3=sectorPerformance(API_key='****************')

def quote(API_key='****************',ticker='SPY'):
  from alpha_vantage.timeseries import TimeSeries
  ts=TimeSeries(key=API_key, output_format='pandas')
...
inf4=quote(API_key='****************',ticker='SPY')
inf5=quote(API_key='****************',ticker='DIA')
def submit1():
    a=textbox.get()
    try:
        timeSeries(API_key='****************', ticker=a)
    except ValueError:
        incorr()
